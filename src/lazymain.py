...
# AI Classification imports - Lazy loading
AI_AVAILABLE = None  # Will be set when first accessed
AI_CLASSIFIER_MODULE = None
...
def load_ai_libraries():
    """Lazy load AI classification libraries when needed."""
    global AI_AVAILABLE, AI_CLASSIFIER_MODULE, AI_CLASSIFICATION_RESULT
    
    if AI_AVAILABLE is None:
        try:
            from ai_failure_classifier import AIClassifier, AIClassificationResult
            AI_CLASSIFIER_MODULE = AIClassifier
            AI_CLASSIFICATION_RESULT = AIClassificationResult
            AI_AVAILABLE = True
            logging.info("AI classification libraries loaded successfully")
        except ImportError:
            AI_AVAILABLE = False
            AI_CLASSIFIER_MODULE = None
            AI_CLASSIFICATION_RESULT = None
...
def get_ai_classifier():
    """Get the AI classifier class if available."""
    if load_ai_libraries():
        return AI_CLASSIFIER_MODULE
    return None

def get_ai_classification_result():
    """Get the AI classification result class if available."""
    if load_ai_libraries():
        return AI_CLASSIFICATION_RESULT
    return None
...
def process_files(work_order_path: str, dict_path: str, status_label: ttk.Label, root: tk.Tk, output_dir: Optional[str] = None, use_ai: bool = False, ai_classifier: Optional[Any] = None, column_mapping: Optional[dict] = None) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if use_ai and load_ai_libraries() and ai_classifier is None:
            try:
                AIClassifier = get_ai_classifier()
                if AIClassifier:
                    ...
                    logging.warning("AI classifier not available, falling back to dictionary matching")
                    use_ai = False
            except Exception as e:
...
class FailureModeApp:
    ...
    def run_processing(self):
        ...
        if self.use_ai_classification and not load_ai_libraries():
        ...
        if self.use_ai_classification:
            try:
                ...
                if AIClassifier:
                    ...
                    self.status_label.config(text="AI classifier not available.", foreground="red")
                    messagebox.showerror("Error", "AI classifier not available. Please install required dependencies.")
                    return
        ...
        def start_batch_processing():
            ...
            def process_files_thread():
                try:
                    ...
                    # Initialize AI classifier if needed
                    ai_classifier = None
                    if use_ai and load_ai_libraries():
                        try:
                            confidence_threshold = float(self.confidence_var.get())
                            ...
                            if AIClassifier:
                                ...
                                results_text.insert(tk.END, "Warning: AI classifier not available\n")
                                use_ai = False
                        except Exception as e:
